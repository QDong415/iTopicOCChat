...
- (void)setup {
    ...
    if (!_shareMenuItemTitleLabel) {
        UILabel *shareMenuItemTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.shareMenuItemButton.frame), kXHShareMenuItemWidth, 17)];
        shareMenuItemTitleLabel.backgroundColor = [UIColor clearColor];
        if (@available(iOS 11.0, *)) {
            shareMenuItemTitleLabel.textColor = [UIColor colorNamed:@"text_black_gray"];
        } else {
            shareMenuItemTitleLabel.textColor = COLOR_BLACK_RGB;
        }
...
- (void)reloadItemOfIndex:(int)index withNormalIconImage:(UIImage *)image withTitle:(NSString *)title {
    if ([self.shareMenuScrollView.subviews count] > index) {
        XHShareMenuItemView *shareMenuItemView = self.shareMenuScrollView.subviews[index];
