...
@interface HolderTextView()<UITextViewDelegate>
...
- (UILabel *)placeHoldLabel {
    if (!_placeHoldLabel) {
        ...
        _placeHoldLabel.textColor = RGBCOLOR(199, 199, 204);
...
- (void)changeSize {
    ...
    if (oriFrame.size.height != sizeToFit.height && sizeToFit.height <= self.maxHeight) {
        ...
        if (self.myDelegate && [self.myDelegate respondsToSelector:@selector(holderTextView:heightChanged:)]) {
            [self.myDelegate holderTextView:self heightChanged:oriFrame];
        }
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if ([text isEqualToString:@"\n"]) {
        if (self.myDelegate && [self.myDelegate respondsToSelector:@selector(holderTextViewEnterDone:)]) {
            [self.myDelegate holderTextViewEnterDone:self];
        }
    ...
    if (kMaxLength > 0 && textView.text.length > kMaxLength){
     ...
     //输入了@
    if ([text isEqualToString:@"@"]) {
        if (self.myDelegate && [self.myDelegate respondsToSelector:@selector(holderTextViewEnterAt:)]) {
            return [self.myDelegate holderTextViewEnterAt:self];
        }
        return YES;
    }
...
- (BOOL)textView:(HolderTextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange {
...
- (BOOL)textView:(HolderTextView *)textView shouldInteractWithTextAttachment:(NSTextAttachment *)textAttachment inRange:(NSRange)characterRange {
...
- (NSMutableArray<NSString *> *)findAtUserids:(NSMutableArray<UserBaseModel *> *)userArray
{
    ...
    NSMutableArray<NSString *> *atUseridArray = [NSMutableArray array];
    [atUseridArray removeAllObjects];
    
    NSError *error = nil;
    NSString *atRegex = @"@([\u4E00-\u9FA5A-Za-z0-9_.-]+)";
    NSRegularExpression *atRegular = [NSRegularExpression regularExpressionWithPattern:atRegex options:NSRegularExpressionCaseInsensitive error:&error];
    
    NSArray *resultArray = [atRegular matchesInString:self.text options:0 range:NSMakeRange(0, self.text.length)];
    for(NSTextCheckingResult *match in resultArray) {
        NSRange range = [match range];
        NSString *name = [self.text substringWithRange:NSMakeRange(range.location + 1, range.length - 1)];
        NSString *userid = [self findUseridByName:name inArray:userArray];
        if (userid && ![atUseridArray containsObject:userid]) {
            [atUseridArray addObject:userid];
        }
    }
    return atUseridArray;
}

- (NSString *)findUseridByName:(NSString *)name inArray:(NSMutableArray<UserBaseModel *> *)userArray
{
    for (UserBaseModel *model in userArray) {
