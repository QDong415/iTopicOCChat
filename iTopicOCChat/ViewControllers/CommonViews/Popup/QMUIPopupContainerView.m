...
@interface QMUIPopContainerViewController : BaseViewController
...
- (void)layoutDefaultSubviews {
    ...
    if (isTextLabelShowing) {
        ...
        CGPoint textLabelOrigin = CGPointMake(textLabelMinX, (CGRectGetHeight(self.contentView.bounds) - ceil(textLabelSize.height))/2 + self.textEdgeInsets.top);
...
- (void)layoutWithTargetView:(UIView *)targetView window:(UIWindow *)window{
    CGRect targetViewFrameInMainWindow = CGRectZero;
    UIWindow *mainWindow = [[[UIApplication sharedApplication] delegate] window];
    if (targetView.window == mainWindow) {
        targetViewFrameInMainWindow = [targetView convertRect:targetView.bounds toView:targetView.window];
    ...
    [self layoutWithTargetRect:targetViewFrameInMainWindow inReferenceWindow:window];
}
...
+ (void)setDefaultAppearance {
    ...
    appearance.maskViewBackgroundColor =  [UIColor colorWithRed:0 green:0 blue:0 alpha:0.35];                 // UIColorMask : 深色的背景遮罩，默认用于 QMAlertController、QMUIDialogViewController 等弹出控件的遮罩;
    appearance.highlightedBackgroundColor = nil;
    appearance.shadowColor =  [UIColor colorWithRed:0 green:0 blue:0 alpha:0.1];
    appearance.borderColor = RGBCOLOR(198, 198, 198);
    appearance.borderWidth = 1 / [[UIScreen mainScreen] scale];
    appearance.cornerRadius = 10;
//    objc_setAssociatedObject(appearance, &kAssociatedObjectKey_outsideEdge, [NSValue valueWithUIEdgeInsets:qmui_outsideEdge], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
