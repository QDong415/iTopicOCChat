...
- (void)layoutSubviews {
    ...
    for (NSInteger section = 0, sectionCount = self.itemSections.count; section < sectionCount; section ++) {
        NSArray<QMUIPopupMenuItem *> *items = self.itemSections[section];
        for (NSInteger row = 0, rowCount = items.count; row < rowCount; row ++) {
            UIButton *button = items[row].button;
            button.frame = CGRectMake(0, minY, contentWidth, self.itemHeight);
            minY = CGRectGetMaxY(button.frame);
            
            BOOL shouldShowSeparatorAtRow = [self shouldShowSeparatorAtRow:row rowCount:rowCount inSection:section sectionCount:sectionCount];
            if (shouldShowSeparatorAtRow) {
                self.itemSeparatorLayers[separatorIndex].frame = CGRectMake(self.separatorInset.left, minY - (1 / [[UIScreen mainScreen] scale]) + self.separatorInset.top - self.separatorInset.bottom, contentWidth - (self.separatorInset.left + self.separatorInset.right), 1 / [[UIScreen mainScreen] scale]);
                separatorIndex++;
            }
        }
    }
...
+ (void)setDefaultAppearanceForPopupMenuView {
    ...
    appearance.separatorColor = COLOR_DIVIDER_RGB;
    appearance.itemTitleFont = [UIFont systemFontOfSize:16];
    appearance.itemHighlightedBackgroundColor = RGBCOLOR(238, 239, 241);
...
+ (instancetype)itemWithImage:(UIImage *)image title:(NSString *)title handler:(void (^)(UIButton *))handler {
    ...
    [button setTitleColor:COLOR_BLACK_RGB forState:UIControlStateNormal];
    if(image){
