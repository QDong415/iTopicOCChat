...
- (NSDictionary *)recordSetting {
    if (!_recordSetting) {
        _recordSetting = [[NSDictionary alloc] init];
        //dq 2020-09-17 经过测试，什么参数都不带，是目前音频质量最好的，以下参数无论怎么设置，都差不多，都很浑浊
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder
    ...
    NSString *recordPath = [[_recorder url] path];//recordPath是mav文件的完整路径
    NSLog(@"audioRecorderDidFinishRecording %@",[recordPath lastPathComponent]);
    if (!flag) {
        ...
        //dq 2020-10-03 如果录制参数hz是8000，那么声音很浑浊，如果不设置参数，那么无法转为正常的amr格式
        //但是经过测试，android 10也能直接播放wav，干脆不转了，直接用wav
...
//         NSLog(@"audioRecorderDidFinishRecording amrFilePath = %@",amrFilePath);
//        BOOL convertResult = [self convertWAV:recordPath toAMR:amrFilePath];
//        if (convertResult) {
//            //上传给服务器时候，我上传的是amr（安卓用的），播放时候，先下载，然后转为wav
//            if ([self.recordDelegate respondsToSelector:@selector(audioRecordDidFinishSuccessed:duration:)]) {
//                [self.recordDelegate audioRecordDidFinishSuccessed:recordPath duration:endDuration];
//            }
//        }else {
...
//    NSString *wavFilePath = amrFileName;
    if (![fileManager fileExistsAtPath:wavFilePath]) {
        NSLog(@"不存在 %@",wavFilePath);
        NSError *error1 = [NSError errorWithDomain:ERROR_AUDIO_DOMAIN
...
//    NSURL *filePath = [amrTmpFolder URLByAppendingPathComponent:filePathName];
     ...
     NSString *filePathName = [NSString stringWithFormat:@"%@.amr", fileName];
     NSURL *filePath = [NSURL URLWithString:[ValueUtil getVoiceLocalPathWithFileKey:filePathName]];
